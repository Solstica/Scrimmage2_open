...
def fit_one_angle(spectrum: dict[str, object]) -> tuple[np.ndarray, list[dict[str, object]]]:
    ...
    for d0, log_n0, n30 in product(np.linspace(6.6, 8.4, 10), (15.5, 18.0, 20.0), (2.35, 2.75, 3.10)):
        initial = np.array([d0, log_n0, n30])
        fit = least_squares(
            lambda p: residual_full(np.r_[p, SELL_REF], spectrum),
            initial, bounds=(STAGE1_LOWER, STAGE1_UPPER),
    ...
    candidates, seen = [], set()
    for row in sorted(stage1, key=lambda item: float(item["rmse"])):
        ...
        if len(candidates) == 8:
            break
    refined = []
    for seed_id, candidate in enumerate(candidates, start=1):
        parameters = np.asarray(candidate["parameters"])
        fit = least_squares(
        ...
        refined.append({
            ...
            "nfev": int(fit.nfev),
            "boundary_hits": boundary_hits(fit.x),
        })
...
def save_angle_assets(
    module: Path, label: str, spectrum: dict[str, object], parameters: np.ndarray,
    multistart: list[dict[str, object]],
) -> dict[str, object]:
        ...
        "multistart": [
            {
                ...
                "initial": {
                    "thickness_um": float(row["initial"][0]),
                    "log10_carrier_cm3": float(row["initial"][1]),
                    "substrate_index": float(row["initial"][2]),
                },
                "parameters": {
                    name: float(value)
                    for name, value in zip(PARAMETER_NAMES, row["parameters"])
                },
                "rmse_fraction": float(row["rmse"]),
                "success": bool(row["success"]),
                "nfev": int(row["nfev"]),
                "boundary_hits": list(row["boundary_hits"]),
            }
...
def main() -> None:
    ...
    angle_results = [
        save_angle_assets(module, label, spectrum, parameters, multistart)
        for (label, _, _), spectrum, (parameters, multistart) in zip(FILES, spectra, fits)
    ]
    ...
    output = args.output.resolve() if args.output else project / "output" / "results" / "q2_paper_a_results.json"
